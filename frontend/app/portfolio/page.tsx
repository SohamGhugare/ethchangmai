  ...
  const fetchBalance = async () => {
    if (account?.address) {
      try {
        ...
        // Generate a random balance between 0.1 and 5 ETH for demo
        const storedBalance = localStorage.getItem('eth_balance');
        const ethBalance = storedBalance || (Math.random() * 4.9 + 0.1).toFixed(4);
        if (!storedBalance) {
  ...
  const handleRequestFaucet = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setFaucetMessage({
          type: 'success',
          text: `Successfully received ${data.amountInETH} ETH! Transaction: ${data.transactionHash.slice(0, 8)}...`,
        });
              ...
              {loadingBalance ? (
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xl md:text-3xl font-bold text-white font-(family-name:--font-space-grotesk)">
                    {balance} <span className="text-base md:text-xl text-green-400">ETH</span>
                  </span>
                  <button
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        ...
                        <span className="px-2.5 py-1 bg-purple-500/20 border border-purple-500/50 rounded-full text-purple-400 text-xs font-semibold font-(family-name:--font-space-grotesk)">
                          ...
                          {formatETH(poll.totalPool)} ETH Pool
                        </span>
                        {poll.is_finalized ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-white/5 rounded-xl p-4">
                      ...
                      <p className="text-white font-semibold font-(family-name:--font-space-grotesk)">
                        {poll.option1Votes} votes • {formatETH(poll.totalOption1Stake)} ETH
                      </p>
                    </div>
                    <div className="bg-white/5 rounded-xl p-4">
                      <p className="text-gray-400 text-sm font-(family-name:--font-space-grotesk) mb-1">
                      ...
                      <p className="text-white font-semibold font-(family-name:--font-space-grotesk)">
                        {poll.option2Votes} votes • {formatETH(poll.totalOption2Stake)} ETH
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-xl p-4">
                      ...
                      <div className="space-y-2">
                        ...
                        {poll.finalization_tx_hash && (
                          <a
                            href={`https://sepolia.etherscan.io/tx/${poll.finalization_tx_hash}`}
                            target="_blank"
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          <span className="px-2.5 py-1 bg-white/10 rounded-full text-gray-300 text-xs font-(family-name:--font-space-grotesk)">
                            Staked: {formatETH(vote.stakeAmount)} ETH
                          </span>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="space-y-2">
                          ...
                          {won && reward > 0 && (
                            <div className="flex items-center gap-2">
                              ...
                              <p className="text-white font-semibold font-(family-name:--font-space-grotesk) text-base md:text-lg">
                                Reward: +{formatETH(reward)} ETH
                              </p>
                            </div>
                          )}
                          {won && (
                            <p className="text-gray-400 text-xs font-(family-name:--font-space-grotesk)">
                              Profit: +{formatETH(reward - vote.stakeAmount)} ETH ({((reward / vote.stakeAmount - 1) * 100).toFixed(2)}%)
                            </p>
                          )}
                          {!won && (
                            <p className="text-gray-400 text-xs font-(family-name:--font-space-grotesk)">
                              Your stake of {formatETH(vote.stakeAmount)} ETH was lost
                            </p>
                          )}
                        </div>
                      </div>
