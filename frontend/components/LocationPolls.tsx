...
export default function LocationPolls({ onCreateClick }: LocationPollsProps) {
      ...
      try {
        ...
        if (data.success && data.polls) {
          ...
          const filtered = data.polls.filter((poll: Poll) => {
            ...
            const isCreator = connected && account && poll.creator === account.address;
  ...
  const handleVoteClick = (poll: Poll, option: number) => {
    ...
    if (poll.creator === account.address) {
  ...
  const handleVoteSubmit = async () => {
    ...
    try {
      ...
      const stakeInWei = Math.floor(stake * 1e18);

      // Submit transaction to the blockchain
      const response = await signAndSubmitTransaction({
        to: votingPoll.creator,
        value: stakeInWei,
        data: {
              ...
              <div className="relative z-10">
                ...
                <div className="mb-6">
                  {(() => {
                    ...
                    // Convert octas to ETH for display (1 ETH = 100,000,000 Octas)
                    const option1ETH = (option1Stake / 100000000).toFixed(2);
                    const option2ETH = (option2Stake / 100000000).toFixed(2);
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex justify-between items-center text-xs text-gray-500 font-(family-name:--font-space-grotesk)">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span>•</span>
                              <span className="whitespace-nowrap">{option1ETH} ETH</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <span className="whitespace-nowrap">{option2ETH} ETH</span>
                              <span>•</span>
                            ...
                            : poll.is_finalized
                              ...
                              : poll.expiryTime < currentTime
                                ...
                                : poll.userVote?.stakeAmount
                                  ? `Already staked ${(poll.userVote.stakeAmount / 100000000).toFixed(2)} ETH`
                                  : "Already voted"
                          }
                        </p>
                ...
                {connected && account && poll.creator === account.address && !poll.is_finalized && (
                  <button
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  {voteTransactionHash && (
                    <div className="space-y-2 mt-2">
                      ...
                      <a
                        href={`https://sepolia.etherscan.io/tx/${voteTransactionHash}`}
                        target="_blank"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-white/5 border border-white/10 rounded-xl">
                      ...
                      <p className="text-white text-2xl font-bold font-(family-name:--font-space-grotesk)">
                        {(pollStats.totalPool / 100000000).toFixed(2)} ETH
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="text-gray-400 text-sm font-(family-name:--font-space-grotesk)">
                          {(pollStats.totalOption1Stake / 100000000).toFixed(2)} ETH
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-gray-400 text-sm font-(family-name:--font-space-grotesk)">
                          {(pollStats.totalOption2Stake / 100000000).toFixed(2)} ETH
                        </p>
                      </div>
                    </div>
                  </div>
                          ...
                          >
                            ...
                            <div className="text-right">
                              <p className="text-white font-semibold text-sm font-(family-name:--font-space-grotesk)">
                                {(reward.reward / 100000000).toFixed(4)} ETH
                              </p>
                              <p className="text-gray-400 text-xs font-(family-name:--font-space-grotesk)">
                                Staked: {(reward.stake / 100000000).toFixed(2)} ETH
                              </p>
                            </div>
                          </div>
                        ))}
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  {endPollTxHash && (
                    <div className="space-y-2 mt-2">
                      ...
                      <a
                        href={`https://sepolia.etherscan.io/tx/${endPollTxHash}`}
                        target="_blank"
                        rel="noopener noreferrer"
